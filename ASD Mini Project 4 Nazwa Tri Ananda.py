...
class JadwalKelas:
    ...
    def Pembaruan_Jadwal(self, Kelas_Terdaftar, Jurusan=None, Matakuliah=None, Date=None, Ruangan=None, Keterangan=None):
        ...
        while current:
            if current.data['Nomor Pendaftaran'] == Kelas_Terdaftar:
                ...
                print("Keterangan:", current.data['Keterangan'])
    ...
    def jump_search(self, arr, x, key):
        ...
        step = int(n ** 0.5)
        prev = 0
        while arr[min(step, n) - 1][key] < x:
            prev = step
            step += int(n ** 0.5)
            if prev >= n:
                return -1
        while arr[prev][key] < x:
            prev += 1
            if prev == min(step, n):
                return -1
        if arr[prev][key] == x:
    ...
    def search_jadwal(self, search_key, search_value):
        search_attributes = {
            1: 'Nomor Pendaftaran',
            2: 'Jurusan',
            3: 'Mata Kuliah',
        ...
        if search_key not in search_attributes:
            print("Atribut pencarian tidak valid.")
            return

        attribute = search_attributes[search_key]

        current = self.Matkul.head
        found = False
        while current:
            if current.data[attribute] == search_value:
                found = True
                print("\nJadwal ditemukan:")
                print("Nomor Pendaftaran:", current.data['Nomor Pendaftaran'])
                print("Jurusan:", current.data['Jurusan'])
...
def main():
    Kelas_Terdaftar = [
        {'Kelas': 'D404', 'available': True},
        {'Kelas': 'D403', 'available': True},
        {'Kelas': 'D402', 'available': True},
        {'Kelas': 'D304', 'available': True},
        {'Kelas': 'D303', 'available': True},
    ...
    while True:
        ...
        print("-------------------- Menu Utama ------------------------")
        print("~" * 56)
        print("---              1. Membuat Jadwal                   ---")
        print("---              2. Memeriksa Jadwal Terdaftar       ---")
        print("---              3. Memperbarui Jadwal               ---")
        print("---              4. Mengurutkan Jadwal Terdaftar     ---")
        print("---              5. Mencari Jadwal                   ---")
        ...
        if Pilih == '1':
            ...
            print("5. Ruangan")
            print("-" * 45)
            search_key = int(input("Masukkan nomor atribut untuk pencarian: "))
            search_value = input("Masukkan nilai yang ingin dicari: ")
            print("-" * 45)
            Sistem_Pemilihan.search_jadwal(search_key, search_value)
            input("\nTekan Enter untuk kembali ke Menu Utama")
            os.system('cls' if os.name == 'nt' else 'clear')
